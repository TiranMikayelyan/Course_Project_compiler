...
## Building for submission (Git)

Commit source and examples; do **not** commit build artifacts:
...
Add a `.gitignore` that excludes `build/`, `out/`, `*.obj`, `*.exe`, `*.exec`, `.vs/`, and `vm_debug.log`. The teacher can build with `build.bat` or CMake, then run the examples.

## License

Course project — use and modify as required by your course policy.
